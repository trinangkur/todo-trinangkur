...
const addTodo = (req, res) => {
  ...
  todoCollection.addTodo(todoCollection.nextTodoId(), new Todo(body.title));
  res.end(todoCollection.toJson());
...
const addTodoItem = (req, res) => {
  const body = querystring.parse(req.body);
  todoCollection.addTodoItem(body.titleId, {
    id: todoCollection.nextItemId(body.titleId),
    text: body.itemText
  });
  res.end(todoCollection.toJson());
  todoCollection.writeTo(fs.writeFileSync, DATA_FILE_PATH);
...
const toggleItemStatus = (req, res) => {
  const body = querystring.parse(req.body);
  todoCollection.toggleItemStatus(body.titleId, body.itemId);
  res.end(todoCollection.toJson());
  todoCollection.writeTo(fs.writeFileSync, DATA_FILE_PATH);
};
